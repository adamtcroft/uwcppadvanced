...
TEST(WriteScene, SceneWriter)
{
    ...
    Xml::ElementCollection childElements = root->getChildElements();
    ...
    Xml::ElementCollection layerElements = child->getChildElements();
    ...
    Xml::ElementCollection placedGraphicElements = layerElements[0]->getChildElements();
    CHECK_EQUAL(1, placedGraphicElements.size());
    Xml::IElement& vectorGraphic = *placedGraphicElements[0];
    CHECK_EQUAL(1, vectorGraphic.getAttributes().size());
    CHECK_EQUAL("true", vectorGraphic.getAttribute("closed"));

    Xml::ElementCollection pointElements = vectorGraphic.getChildElements();
